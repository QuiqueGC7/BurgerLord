...
function renderizarMenu(data) {
    ...
    // Filtrar solo los productos cuyo Product_Type_ID sea 1, 2, 3 o 4
    ...
    // Agrupar productos por Product_Type_ID
    const productosPorTipo = {};
    filteredData.forEach(item => {
        if (!productosPorTipo[item.Product_Type_ID]) {
            productosPorTipo[item.Product_Type_ID] = [];
        }
        productosPorTipo[item.Product_Type_ID].push(item);
    });

    // Seleccionar un producto aleatorio de cada tipo
    const productosAleatorios = [];
   

    // Renderizar los productos seleccionados
    productosAleatorios.forEach(item => {
